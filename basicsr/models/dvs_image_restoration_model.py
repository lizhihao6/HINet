...
class DVSImageRestorationModel(BaseModel):
    ...
    def grids(self):
        ...
        while i < h and not last_i:
            ...
            while j < w and not last_j:
                ...
                for trans_idx in range(self.opt['val'].get('trans_num', 1)):
                    ...
                    parts_events.append(self.transpose(self.events[:, :, i:i + crop_size, j:j + crop_size], trans_idx))
        ...
        self.events = torch.cat(parts_events, dim=0)
    ...
    def _get_preds(self, preds):
        ...
        #     preds = [torch.pow(torch.clamp(preds, 0, 1), 1/2.2)]
        # else:
        #     preds = [torch.pow(torch.clamp(p, 0, 1), 1/2.2) for p in preds]
        if not isinstance(preds, list):
            preds = [preds]
        return preds
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger,
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            print('lq', self.lq.max(), self.lq.min(), self.lq.min())
            print('gt', self.gt.max(), self.gt.min(), self.gt.mean())
