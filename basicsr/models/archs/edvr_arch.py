...
class EDVR(nn.Module):
    ...
    def __init__(self,
        ...
        # may be use events denoiser here
        self.events_conv_before = nn.Conv2d(num_events_ch, num_feat, 3, 1, 1)
        self.events_conv_after = nn.Conv2d(num_events_ch, num_feat, 3, 1, 1)
    ...
    def forward(self, x, before_events, after_events):
        b, c, h, w = x.size()
        t = self.num_frame
        ...
        # L1
        if self.with_predeblur:
            feat_l1 = self.conv_1x1(self.predeblur(x))
            if self.hr_in:
                h, w = h // 4, w // 4
        else:
            feat_l1 = self.lrelu(self.conv_first(x))

        # extract events features
        events_feat_before = self.events_conv_before(before_events)
        events_feat_after = self.events_conv_after(after_events)

        # concat image and events features
        feat_l1 = torch.cat((events_feat_before, feat_l1, events_feat_after))
