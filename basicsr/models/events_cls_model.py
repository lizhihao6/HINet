...
class TransClsModel(BaseModel):
    ...
    def __init__(self, opt):
        super(TransClsModel, self).__init__(opt)
    ...
    def split(self):
        ...
        if len(parts) == 0:  # n <= crop_size
            part_events = self.events
            part_events[:, :, 2] = (part_events[:, :, 2] - part_events[:, 0, 2]) / (
                    part_events[:, -1, 2] - part_events[:, 0, 2])
            if part_events.shape[1] < crop_size:
                pad_before = (crop_size - part_events.shape[1]) // 2
                part_events = torch.nn.functional.pad(part_events.transpose(1, 2),
                                                      (pad_before, crop_size - part_events.shape[1] - pad_before),
                                                      'constant').transpose(1, 2)
            parts.append(part_events)
