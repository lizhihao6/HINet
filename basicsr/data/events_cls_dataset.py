...
class EventsClsDataset(data.Dataset):
    def __init__(self, opt):
        super(EventsClsDataset, self).__init__()
        self.opt = opt
    ...
    def _get_events(self, key):
        ...
        # padding, maybe use other padding method, all padding will be done in events_cls_model.py
        if sample_size < target_size and self.sample_fn != 'all':
            pad_before = (target_size - sample_size) // 2
            events = np.pad(events, ((pad_before, target_size - sample_size - pad_before), (0, 0)), 'constant')
